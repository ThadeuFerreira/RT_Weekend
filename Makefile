...
debug-tsan:
	mkdir -p build
	odin build . -collection:RT_Weekend=. -debug -sanitize:thread -define:ODIN_TSAN=true -define:TRACE_CAPTURE_ENABLED=true -define:TRACK_ALLOCATIONS=true -define:ENABLE_GL_DEBUG_CALLBACK=false -out:build/debug-tsan
